...
**什麼都存得進去，程式不擋。** 拼得出來就存得下來，匯入也只檢查資料形狀，不檢查「國語有沒有這個音」。

以前擋過，用的判準是「代讀漢字表查不查得到」。那個判準是錯的，因為**那張表本身有洞**：

- `ㄅㄞˊ`（白）：白是破音字（ㄅㄛˊ／ㄅㄞˊ），舊的生成規則只認辭典的**第一讀音**，
  而白的第一讀音是 ㄅㄛˊ、那格又輸給「帛」，於是 `ㄅㄞˊ` 整格不存在——小朋友打不出「白」。
- `ㄉㄧ˙`（弟弟的第二個字）：辭典只收 吧／嗎／的 這類**本身就是輕聲詞**的字，
  但語言事實是**任何音節都可以在詞裡讀輕聲**（弟弟、媽媽、東西、椅子），表裡不可能列得完。

所以「查不到」現在只影響**發音走哪條路**，不影響存不存得進去：

| 情況 | 單張念（點卡／Enter） | 整句念（▶▶） |
|---|---|---|
| 表裡有這個音節 | 用代讀漢字 | 用代讀漢字 |
| 表裡沒有 | 送**注音字串**給引擎（能解析就念對，不能就退化成逐符號念） | 用**同一個 base 任一聲調**的漢字近似（**聲調會是錯的**） |
| 連 base 都沒有（例：ㄈㄞ 這種國語沒有的音） | 逐符號念 | 這一張不放進句子（沒有東西可念） |

整句那格為什麼不能也送注音字串：整句是**一個** utterance，中間混一段注音字串會把連讀打斷。
寧可聲調近似，也要保住「一句話」。

### 輕聲：點在上面

課本的排法是**輕聲的「˙」標在音節正上方，ˊ ˇ ˋ 標在右側**。程式照做：
按下 `7`（或點符號帶上的 ˙），字格的聲調格會**滑到三格正上方**變成一個扁格；
換成別的聲調又滑回右邊。卡片上也一樣，點在符號堆疊的上方置中。

那個上方的格子是浮著的（`position: absolute`），**不佔版面高度**——
否則三格會被往下推、跟右邊三條符號帶的中線就對不上了。
...
所以「整字 token」改查 `src/lib/syllable-reading.ts`（1331 個音節 → 一個代讀漢字），
...
挑字規則（優先序）：**單音字**（`heteronyms` 長度 = 1，唸法唯一）→ 該音節是某個破音字的**第一讀音**
→ 該音節是某個破音字的**其他讀音**；同一層裡簡編本收錄的常用字優先、筆畫少者優先。
再加上 16 個由 PCM 稽核修正的例外（原本挑到的字會被引擎唸錯，例如「干」被唸成 ㄍㄢˋ）。

第五期把「其他讀音」那一層加進來，表從 **1250 → 1331**（＋81，沒有任何既有條目被改動），
`ㄅㄞˊ → 白` 就是這樣補回來的。破音字的非第一讀音有「引擎照它的第一讀音念」的風險，
所以新增的 81 個全部跑過 PCM 稽核（詳見 `note/注音卡拉OK-第五期-實作回報-0905.md`）。
